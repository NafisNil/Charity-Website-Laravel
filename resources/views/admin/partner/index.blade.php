                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <td><a href="{!! $item->url !!}"></a> {!! $item->url !!}</td>
                   ...
                   <td>
                      ...
                      <a href="{{route('partner.edit',[$item])}}" title="Edit"><button class="btn btn-outline-info btn-sm"><i class="fas fa-pen-square"></i></button></a>

                        <button class="btn btn-outline-danger btn-sm" title="Delete" onclick="confirmDelete({{ $item->id }})"><i class="fas fa-trash"></i></button>
                        <form id="delete-form-{{ $item->id }}" action="{{route('partner.destroy',[$item])}}" method="POST" style="display:none;">
                          @method('DELETE')
                          @csrf
                        </form>
